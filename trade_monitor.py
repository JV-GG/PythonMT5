...
def _process_one_position(pos: dict) -> None:
    ...
    if phase == PHASE_INITIAL:
        ...
        if triggered_60:
            ...
            # 2. Check 30% trigger (Move SL to Breakeven / Entry)
            ...
            if triggered_30:
                ...
                # 3. Check 20% trigger (Cut SL risk by 50%)
    ...
    if phase == PHASE_INITIAL:
        ...
        if triggered_60:
            ...
            if triggered_50:
                ...
                sl_better = (current_sl == 0.0) or (lock_50_sl > current_sl if is_buy else lock_50_sl < current_sl)
                if sl_better and _is_valid_sl(symbol, direction, lock_50_sl):
                ...
                if triggered_40:
                    ...
                    sl_better = (current_sl == 0.0) or (lock_40_sl > current_sl if is_buy else lock_40_sl < current_sl)
                    if sl_better and _is_valid_sl(symbol, direction, lock_40_sl):
...
>>>>>>> f69ed39 (Add 40% (lock 5%) and 50% (lock 10%) early SL profit lock steps)
